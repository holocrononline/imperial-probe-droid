...
def get_full_avatar_url(config, image, unit):
	...
	alignment = BaseUnit.get_alignment(image)
	...
	return 'http://%s/avatar/%s?level=%s&gear=%s&rarity=%s&zetas=%s&relics=%s&alignment=%s&version=5' % (config['server'], image, level, gear, rarity, zetas, relics, alignment)
